...
export function createStyleCompilation(ctx: CompilationContext) {
  ...
  function compileViaPipeline(
    ...
    const atomic = result.ir.rules.filter(
    ...
    if (ctx.prefixer && ctx.config.prefixer.enabled && finalCss.trim()) {
      try {
        finalCss =
          (ctx.prefixer as any).lightweightPrefix(finalCss) || finalCss;
      } catch (e) {
        ctx.emit({
          type: "warning",
    ...
    (out as any).inspector = {
      ir: result.ir,
      pipelineReport: result.timeline,
      diagnostics: result.ir.diagnostics,
    };
