...
import { DEFAULT_CONFIG, PERFORMANCE } from './constants.js'
...
import { StyleGraphCompiler, GraphCompileOptions } from '../compiler/style-graph.js'
...
import { StatsTracker } from './compiler/stats.js'
...
export class ChainCSSCompiler {
  ...
  constructor(config: ChainCSSConfig) {
    ...
    // FIX: Do NOT rmSync .chaincss-cache here — let CacheManager handle it
    try {
  ...
  // Events — FIX: emit to both CompilerEvents and handlers
  ...
  public compileStyle(id: string, def: StyleDefinition): CompileResult {
    ...
    const nestedRules = (def as any)._nestedRules || []
    const atRules = (def as any)._atRules || []
  ...
  // Batch — FIX: proper queue that runs all jobs
  ...
  // File single
  public async compileFile(filePath: string) {
    await this.ready()
    const ex = await this.loader.import(filePath)
    const out: Record<string, CompileResult> = {}
    for (const [n, v] of Object.entries(ex)) {
      if (typeof v === 'function' && (v as any).variants) out[n] = this.compileRecipe(n, v)
      else if ((v as any)?.selectors) out[n] = this.compileStyle(n, v as any)
    }
    return out
  }
  ...
  // Graph
  public compileWithGraph(styles: Record<string, StyleDefinition>, opts?: GraphCompileOptions) {
    return new StyleGraphCompiler({ ...opts, verbose: this.config.verbose }).compile(styles)
  }
  ...
  private computeStats() {
    ...
    const atomic = rules.filter((r: any) => r.meta?.atomic === true).length
    return {
      totalStyles: total,
      atomicStyles: atomic,
      uniqueProperties: 0,
  ...
  // State / CSS Accumulation — FIX: cached join
