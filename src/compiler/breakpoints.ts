...
export function generateResponsiveCSS(
  ...
  for (const bp of sortedKeys) {
    ...
    if (!bStyles) continue;
    const query = _current[bp];
    if (!query) continue;
    const media =
      query.startsWith("(") || query.startsWith("print")
        ? `@media ${query}`
        : `@media ${query}`;
    css += `${media} {\n ${selector} {\n`;
...
export function responsive<T>(
  ...
  if (value && typeof value === "object" && !Array.isArray(value)) {
    ...
    if (keys.some((k) => k in _current || k === "base"))
