...
export type CompilerEventHandler<K extends keyof CompilerEventMap> = (payload: CompilerEventMap[K]) => void;

// Clean interface implementation layout for typed event handlers
export interface CompilerEvents {
  on<K extends keyof CompilerEventMap>(event: K, listener: CompilerEventHandler<K>): this;
  once<K extends keyof CompilerEventMap>(event: K, listener: CompilerEventHandler<K>): this;
  off<K extends keyof CompilerEventMap>(event: K, listener: CompilerEventHandler<K>): this;
  emit<K extends keyof CompilerEventMap>(event: K, payload: CompilerEventMap[K]): boolean;
}
...
export class CompilerEvents extends EventEmitter {
  ...
  emitError(error: Error, filePath?: string): void { 
    ...
    if (this.listenerCount('error') === 0) {
      console.error(`[ChainCSS Error] ${filePath ? `(${filePath}): ` : ''}${error.stack || error.message}`);
      return;
    }
