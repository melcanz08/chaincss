...
import { parseColor as parseColorBase, contrastRatio as baseContrastRatio } from './design-orchestrator.js'
...
export interface DerivedRelationship { type: 'derived'; id?: string; source: TokenPath; target: TokenPath; method: DerivedMethod }
export interface HarmonyRelationship { type: 'harmony'; id?: string; source: TokenPath; targets: TokenPath[]; rule: 'complementary' | 'analogous' | 'triadic' | 'same-lightness' }
export type Relationship = ContrastRelationship | DerivedRelationship | HarmonyRelationship
export interface Violation { relationship: Relationship; foreground: string; background: string; ratio: number; target: number; message: string }
export interface Change { path: TokenPath; from: string; to: string; reason: string; method?: string; ratio?: number }
export interface EntanglementReport { valid: boolean; violations: Violation[]; changes: Change[]; tokens: Record<string, any> }
...
function mixOklch(a: RGB, b: RGB, t: number): RGB {
  const ca = rgbToOklch(a), cb = rgbToOklch(b)
  let dh = cb.h - ca.h
  if (dh > 180) dh -= 360
  if (dh < -180) dh += 360
  return oklchToRgb({ l: ca.l + (cb.l - ca.l) * t, c: ca.c + (cb.c - ca.c) * t, h: (ca.h + dh * t + 360) % 360 })
}
function findClosestFix(fgStr: string, bgStr: string, target = 4.5, method: ContrastMethod = 'auto'): { fixed: string; ratio: number; fixMethod: string } | null {
  ...
  let lo = shouldDarken ? 0 : fgOklch.l, hi = shouldDarken ? fgOklch.l : 1
  ...
  return { fixed: rgbToHex(fixedRgb.r, fixedRgb.g, fixedRgb.b), ratio: Math.round(best.ratio * 100) / 100, fixMethod: shouldDarken ? 'darken' : 'lighten' }
}
...
function applyHarmony(sourceStr: string, rule: HarmonyRelationship['rule'], index: number): string | null {
...
function cloneTokens<T>(t: T): T {
  ...
  } catch { return JSON.parse(JSON.stringify(t)) }
}
function topoSort(derived: DerivedRelationship[], harmonies: HarmonyRelationship[]) {
  type Flat = DerivedRelationship | { type: 'harmony_single'; source: string; target: string; rule: HarmonyRelationship['rule']; index: number; total: number }
  const graph = new Map<string, Set<string>>()
  const indeg = new Map<string, number>()
  const flat: Flat[] = []
  const touch = (n: string) => { if (!indeg.has(n)) indeg.set(n, 0) }
  for (const r of derived) {
    flat.push(r); touch(r.source); indeg.set(r.target, (indeg.get(r.target) || 0) + 1)
    if (!graph.has(r.source)) graph.set(r.source, new Set()); graph.get(r.source)!.add(r.target)
  }
  for (const h of harmonies) {
    h.targets.forEach((target, index) => {
      const single: Flat = { type: 'harmony_single', source: h.source, target, rule: h.rule, index, total: h.targets.length }
      flat.push(single); touch(h.source); indeg.set(target, (indeg.get(target) || 0) + 1)
      if (!graph.has(h.source)) graph.set(h.source, new Set()); graph.get(h.source)!.add(target)
    })
  }
  const q: string[] = [...indeg.entries()].filter(([, d]) => d === 0).map(([n]) => n)
  const order: string[] = []
  while (q.length) {
    const n = q.shift()!; order.push(n)
    for (const o of graph.get(n) || []) {
      const nd = (indeg.get(o) || 1) - 1; indeg.set(o, nd); if (nd === 0) q.push(o)
    }
  }
  if (order.length !== indeg.size) throw new Error(`[entanglement] Cycle detected: ${[...indeg.entries()].filter(([, d]) => d > 0).map(([n]) => n).join(', ')}`)
  const pos = new Map(order.map((n, i) => [n, i]))
  return flat.sort((a, b) => (pos.get(a.source) ?? 0) - (pos.get(b.source) ?? 0))
}

export class TokenEntanglementEngine {
  private relationships: Relationship[] = []
  constructor(relationships: Relationship[] = []) { this.relationships = relationships }
  ...
  validate(tokens: Record<string, any>): Violation[] {
    ...
    for (const rel of this.relationships) {
      ...
      const fgVal = getByPath(tokens, rel.foreground); if (!fgVal) continue
      const bgs = Array.isArray(rel.background) ? rel.background : [rel.background]
      const target = rel.target ?? 4.5
      for (const bgPath of bgs) {
        const bgVal = getByPath(tokens, bgPath); if (!bgVal) continue
        const ratio = baseContrastRatio(String(fgVal), String(bgVal))
        if (ratio < target && ratio !== -1) v.push({ relationship: rel, foreground: String(fgVal), background: String(bgVal), ratio: Math.round(ratio * 100) / 100, target, message: `${rel.foreground} on ${bgPath} is ${ratio.toFixed(2)}:1, needs ${target}:1` })
      }
    }
    return v
  }
  propagate(tokens: Record<string, any>, sourcePath: TokenPath, newValue: string, opts: { autoFixContrast?: boolean } = {}): EntanglementReport {
    ...
    if (from && from !== newValue) changes.push({ path: sourcePath, from, to: newValue, reason: 'source change' })
    const derived = this.relationships.filter(r => r.type === 'derived') as DerivedRelationship[]
    const harmonies = this.relationships.filter(r => r.type === 'harmony') as HarmonyRelationship[]
    const pipeline = topoSort(derived, harmonies)
    let mutated = true, iter = 0
    while (mutated && iter < 25) {
      mutated = false; iter++
      for (const edge of pipeline) {
        const srcVal = getByPath(next, edge.source); if (!srcVal) continue
        let computed: string | null = null, reason = ''
        if ('method' in edge) { computed = applyDerived(String(srcVal), edge.method); reason = `derived from ${edge.source} via ${edge.method}` }
        else {
          if (edge.rule === 'same-lightness') {
            const curTarget = getByPath(next, edge.target)
            const srcRgb = parseColor(String(srcVal)), tgtRgb = parseColor(String(curTarget || '#ffffff'))
            if (srcRgb && tgtRgb) {
              const srcO = rgbToOklch(srcRgb), tgtO = rgbToOklch(tgtRgb)
        ...
        changes.push({ path: edge.target, from: String(cur ?? ''), to: computed, reason, method: 'method' in edge ? edge.method : edge.rule })
        mutated = true
      }
    }
    if (opts.autoFixContrast !== false) {
      ...
      for (const rel of contrastRels) {
        const bgs = Array.isArray(rel.background) ? rel.background : [rel.background]
        const target = rel.target ?? 4.5
        let worstRatio = Infinity, worstBg = ''
        for (const bgPath of bgs) {
          const fgVal = getByPath(next, rel.foreground), bgVal = getByPath(next, bgPath); if (!fgVal || !bgVal) continue
          const ratio = baseContrastRatio(String(fgVal), String(bgVal)); if (ratio === -1) continue
          if (ratio < worstRatio) { worstRatio = ratio; worstBg = bgPath }
        }
        if (worstRatio >= target) continue
        const fgVal = getByPath(next, rel.foreground), bgVal = getByPath(next, worstBg)
        const fix = findClosestFix(String(fgVal), String(bgVal), target, rel.autoFix ?? 'auto')
        if (!fix) continue
        setByPath(next, rel.foreground, fix.fixed)
        changes.push({ path: rel.foreground, from: String(fgVal), to: fix.fixed, reason: `auto fix contrast ${worstRatio.toFixed(2)} -> ${fix.ratio} on ${worstBg}`, method: fix.fixMethod, ratio: fix.ratio })
      }
    }
  ...
  fixAll(tokens: Record<string, any>): EntanglementReport {
    const next = cloneTokens(tokens)
    const changes: Change[] = []
    const rels = (this.relationships.filter(r => r.type === 'contrast') as ContrastRelationship[]).sort((a, b) => (b.priority ?? 0) - (a.priority ?? 0))
    for (const rel of rels) {
      const bgs = Array.isArray(rel.background) ? rel.background : [rel.background]
      for (const bgPath of bgs) {
        const fgVal = getByPath(next, rel.foreground), bgVal = getByPath(next, bgPath); if (!fgVal || !bgVal) continue
        const ratio = baseContrastRatio(String(fgVal), String(bgVal)); if (ratio >= (rel.target ?? 4.5) || ratio === -1) continue
        const fix = findClosestFix(String(fgVal), String(bgVal), rel.target ?? 4.5, rel.autoFix ?? 'auto'); if (!fix) continue
        setByPath(next, rel.foreground, fix.fixed)
...
export function createEntanglementEngine(config: { relationships: Relationship[] }) { return new TokenEntanglementEngine(config.relationships) }