...
export class Theme {
  ...
  get(path: string): string | undefined {
    ...
    for (const p of parts) { 
      if (cur == null || typeof cur !== 'object') { this.cache.set(path, undefined); return undefined; } 
      cur = cur[p]; 
    }
    ...
    const flatten = (obj: ThemeTokens, p = '') => {
      for (const [k, v] of Object.entries(obj)) {
        ...
        if (v && typeof v === 'object') { 
          ...
          const val = typeof v === 'string' && v.startsWith('$') 
...
export function validateTheme(contract: ThemeContract, theme: ThemeTokens = {}, path='', options?: { strict?: boolean }): boolean {
    ...
    if (typeof cPart === 'object' && cPart !== null && typeof cPart !== 'string') {
      ...
      if (options?.strict) {
        for (const k of present) { 
          if (!(k in cPart)) errors.push(`  ✗ Extra token not in strict contract: "${curPath ? `${curPath}.${k}` : k}"`); 
        }
      } else {
        for (const k of present) { 
          if (!(k in cPart)) warnings.push(`  ⚠ Extra token not in contract: "${curPath ? `${curPath}.${k}` : k}"`); 
        }
      }
    } else {
      if (tPart !== undefined && typeof tPart !== 'string' && typeof tPart !== 'number') { 
        errors.push(`  ✗ Token "${curPath}" must be string|number, got ${typeof tPart}`); 
      }
    }
  }
  ...
  function build(cPart: T, tPart: ThemeTokens | undefined, target: ThemeTokens) {
    for (const k of Object.keys(cPart)) {
      if (typeof cPart[k] === 'object' && cPart[k] !== null) {
        target[k] = {}; 
        build(cPart[k] as any, (tPart?.[k] as ThemeTokens) || {}, target[k] as ThemeTokens);
      } else {
