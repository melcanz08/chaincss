...
function setCache(key: string, val: { r: number; g: number; b: number; a: number } | null) {
...
function oklchToRgb(l: number, c: number, h: number): { r: number; g: number; b: number } {
  ...
  const s__ = l_ - 0.0894841775 * a_ - 1.2914855480 * b_;
...
export function parseColor(color: string): { r: number; g: number; b: number; a: number } | null {
  ...
  const hexMatch = trimmed.match(/^#([a-f0-9]{3}|[a-f0-9]{4}|[a-f0-9]{6}|[a-f0-9]{8})$/);
  if (hexMatch) {
    let hex = hexMatch[1];
    ...
    if (hex.length === 6) {
      ...
      out = { r: parseInt(hex.slice(0, 2), 16), g: parseInt(hex.slice(2, 4), 16), b: parseInt(hex.slice(4, 6), 16), a: parseInt(hex.slice(6, 8), 16) / 255 };
    }
  }

  // 2. RGB/RGBA
  if (!out && trimmed.startsWith('rgb')) {
    const rgbMatch = trimmed.match(/rgba?\(\s*([\d.]+)(%?)\s*[\s,]\s*([\d.]+)(%?)\s*[\s,]\s*([\d.]+)(%?)(?:\s*[\s,\/]\s*([\d.]+)(%?))?\s*\)/);
    if (rgbMatch) {
      const parseVal = (val: string, isPct: string, max: number) => isPct? (parseFloat(val) / 100) * max : parseFloat(val);
      ...
      let a = 1;
      if (rgbMatch[7]) {
        a = rgbMatch[8]? parseFloat(rgbMatch[7]) / 100 : parseFloat(rgbMatch[7]);
      }
  ...
  if (!out && trimmed.startsWith('hsl')) {
    const hslMatch = trimmed.match(/hsla?\(\s*([\d.]+)(deg|rad|turn)?\s*[\s,]\s*([\d.]+)%\s*[\s,]\s*([\d.]+)%(?:\s*[\s,\/]\s*([\d.]+)(%?))?\s*\)/);
    if (hslMatch) {
      ...
      let a = 1;
      if (hslMatch[5]) a = hslMatch[6]? parseFloat(hslMatch[5]) / 100 : parseFloat(hslMatch[5]);
      ...
      out = { r: Math.round(r * 255), g: Math.round(g * 255), b: Math.round(b * 255), a };
    }
  }

  // 4. OKLCH
  if (!out && trimmed.startsWith('oklch')) {
    const oklchMatch = trimmed.match(/oklch\(\s*([\d.]+)(%?)\s+([\d.]+)\s+([\d.]+)(deg|rad|turn)?(?:\s*[\s\/]\s*([\d.]+)(%?))?\s*\)/);
    if (oklchMatch) {
      let l = parseFloat(oklchMatch[1]); if (oklchMatch[2]) l /= 100;
      const c = parseFloat(oklchMatch[3]);
      let h = parseFloat(oklchMatch[4]); const unit = oklchMatch[5];
      if (unit === 'rad') h = (h * 180) / Math.PI; else if (unit === 'turn') h = h * 360;
      h = ((h % 360) + 360) % 360;
      let a = 1; if (oklchMatch[6]) a = oklchMatch[7]? parseFloat(oklchMatch[6]) / 100 : parseFloat(oklchMatch[6]);
  ...
  if (!out) {
    const named: Record<string, [number, number, number]> = {
      ...
      blue: [0, 0, 255], orange: [255, 165, 0], yellow: [255, 255, 0], purple: [128, 0, 128],
      pink: [255, 192, 203], brown: [165, 42, 42], navy: [0, 0, 128], teal: [0, 128, 128],
      cyan: [0, 255, 255], magenta: [255, 0, 255], lime: [0, 255, 0], maroon: [128, 0, 0],
      olive: [128, 128, 0], silver: [192, 192, 192], gold: [255, 215, 0], coral: [255, 127, 80],
      salmon: [250, 128, 114], gray: [128, 128, 128], grey: [128, 128, 128], slategray: [112, 128, 144],
      indigo: [75, 0, 130], turquoise: [64, 224, 208], violet: [238, 130, 238], tomato: [255, 99, 71],
      royalblue: [65, 105, 225], steelblue: [70, 130, 180], skyblue: [135, 206, 235],
      forestgreen: [34, 139, 34], darkorange: [255, 140, 0]
    };
    if (named[trimmed]) {
...
export function checkContrast(foreground: string, background: string): ContrastResult | null {
  ...
  return {
    foreground, background, ratio: roundedRatio,
    passes: { AA: ratio >= 4.5, AALarge: ratio >= 3, AAA: ratio >= 7, AAALarge: ratio >= 4.5 },
    suggestion: ratio < 4.5? `Contrast ${roundedRatio.toFixed(2)} fails AA. Darken/lighten by ~${Math.round((4.5 - ratio) * 10)}%` : undefined
  };
}
...
export function auditContrast(styles: Array<{ selector: string; color: string; backgroundColor: string }>): ContrastReport {
  ...
  return {
    ...
    summary: failures.length === 0? `All ${checks.length} checks pass AA.` : `${failures.length} of ${checks.length} FAIL AA.`
  };
}
...
export function resolveContextual(token: ContextualToken, selectorPath: string): string {
  ...
  const targetSegments = selectorPath.split(/[\s>+~]+/).map(s => s.trim()).filter(Boolean);
  for (const [ctx, val] of Object.entries(token.contexts)) {
    const ctxSegments = ctx.trim().split(/[\s>+~]+/).map(s => s.trim()).filter(Boolean);
    let targetIdx = 0, matchCount = 0;
    for (const ctxSeg of ctxSegments) {
      while (targetIdx < targetSegments.length) {
        if (segmentMatches(targetSegments[targetIdx], ctxSeg)) { matchCount++; targetIdx++; break; }
        targetIdx++;
      }
    }
    if (matchCount === ctxSegments.length) {
      const score = ctxSegments.length;
      if (score > highestSpecificity) { highestSpecificity = score; best = val; }
    }
  }
  return best;
}
...
function resolveNestedTokenPath(tokens: Record<string, any>, path: string): string | null {
...
export function validateTokenRelationships(tokens: Record<string, any>, pairs: Array<{ foreground: string; background: string; label: string }>): ContrastReport {
  ...
  for (const pair of pairs) {
    ...
    if (fg && bg) styles.push({ selector: pair.label, color: fg, backgroundColor: bg });
  }
...
export const orchestrator = {
  contrastRatio, checkContrast, auditContrast, createContextualToken,
  resolveContextual, generateContextualCSS, validateTokenRelationships,
  parseColor, importFigmaTokens, clearParseCache
};
