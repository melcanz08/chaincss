...
const DEFAULT_THEME: Record<string, Record<string, SemanticMapping>> = {
  ...
  text: {
    primary: {
      properties: { color: "$colors.gray.900", fontWeight: "400" },
      description: "Primary body text — main content",
    },
    secondary: {
      properties: { color: "$colors.gray.600", fontWeight: "400" },
    ...
    inverse: {
      properties: { color: "$colors.white", fontWeight: "500" },
  ...
  elevation: {
    flat: {
      properties: { boxShadow: "none", zIndex: "0" },
      description: "Flat — no elevation, base level",
    },
    raised: {
      properties: { boxShadow: "$shadows.sm", zIndex: "10" },
      description: "Raised — subtle lift, cards on light backgrounds",
    },
    floating: {
      properties: { boxShadow: "$shadows.md", zIndex: "20" },
    ...
    overlay: {
      properties: { boxShadow: "$shadows.xl", zIndex: "50", position: "fixed" },
  ...
  state: {
    hover: {
      properties: { filter: "brightness(1.1)", transition: "filter 0.2s ease" },
      pseudoClass: "hover",
    ...
    loading: {
      properties: { cursor: "wait", opacity: "0.7", pointerEvents: "none" },
...
const DARK_OVERRIDES: Record<
  string,
  Record<string, Partial<Record<string, string | number>>>
> = {
  surface: {
    interactive: {
...
const HIGH_CONTRAST_OVERRIDES: Record<
  string,
  Record<string, Partial<Record<string, string | number>>>
> = {
  surface: {
