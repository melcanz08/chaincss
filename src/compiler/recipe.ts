...
function deepMergeStyles(target: Record<string, any>, source: Record<string, any>): Record<string, any> {
  ...
  for (const [key, value] of Object.entries(source)) {
    if (key === 'selectors') {
      const targetSelectors = Array.isArray(output.selectors) ? output.selectors : [];
...
export function recipe<TVariants extends Record<string, Record<string, StyleDefinition | (() => StyleDefinition)>>>(
  ...
  const pick = (selection: Partial<{ [K in keyof TVariants]: keyof TVariants[K] }> = {}): StyleDefinition => {
    const selected = { ...defaultVariants, ...selection } as Record<string, any>;
    let merged: StyleDefinition = { selectors: [] } as unknown as StyleDefinition;
    ...
    for (const cv of compoundStyles) {
      ...
      if (
        conditions.length > 0 &&
        conditions.every(([key, value]) => value !== undefined && selected[key] === value)
      ) {
  ...
  let allVariantsCache: Array<Partial<{ [K in keyof TVariants]: keyof TVariants[K] }>> | null = null;

  const getAllVariants = (): Array<Partial<{ [K in keyof TVariants]: keyof TVariants[K] }>> => {
    if (allVariantsCache) return allVariantsCache;

    const result: Array<Partial<{ [K in keyof TVariants]: keyof TVariants[K] }>> = [];
    ...
    function generate(current: Record<string, any>, index: number): void {
      if (index === variantKeys.length) {
        if (result.length >= 1000) {
          throw new Error('Recipe variant cross-product limit exceeded (1000 combinations max).');
        }
