...
import type { AnalysisPass, AnalysisResult, AnalysisAnnotation, AnalysisContext } from '../pipeline-types.js';
  ...
  for (const decl of (rule.declarations || [])) {
    ...
    if (isGridProp && typeof decl.value === 'string') {
      ...
      if (repeatMatch) colCount = parseInt(repeatMatch[1], 10);
        ...
        colCount = sanitizedValue.split(/\s+/).filter((c: string) => c.includes('fr') || c.includes('px') || c.includes('%') || c === 'X').length;
      }
      if (colCount > MAX_GRID_COLUMNS) {
        issues.push({
          ruleId: rule.id, selector: rule.selector, property: decl.property,
          currentValue: decl.value, severity: colCount >= 4? 'error' : 'warning', category: 'grid',
  ...
  for (const decl of (rule.declarations || [])) {
    if ((decl.property === 'fontSize' || decl.property === 'font-size') && typeof decl.value === 'string') {
      ...
      if (px > LARGE_FONT_THRESHOLD) {
        ...
        issues.push({
          ruleId: rule.id, selector: rule.selector, property: decl.property,
          currentValue: decl.value, severity: 'warning', category: 'typography',
...
function detectViewportUnits(rule: IRRule): ResponsiveIssue[] {
  ...
  for (const decl of (rule.declarations || [])) {
    ...
    if (cat === 'viewport' || raw.includes('vh') || raw.includes('dvh') || raw.includes('svh')) {
      if (raw.includes('100vh')) {
        issues.push({
          ruleId: rule.id, selector: rule.selector, property: decl.property,
          currentValue: decl.value, severity: 'warning', category: 'viewport',
...
function detectLargePadding(rule: IRRule): ResponsiveIssue[] {
  ...
  for (const decl of (rule.declarations || [])) {
    ...
    if (px > LARGE_PADDING_THRESHOLD) {
      issues.push({
        ruleId: rule.id, selector: rule.selector, property: decl.property,
        currentValue: decl.value, severity: px > 80? 'error' : 'warning', category: 'spacing',
...
function detectLargeGap(rule: IRRule): ResponsiveIssue[] {
  ...
  for (const decl of (rule.declarations || [])) {
    ...
    if (px > LARGE_GAP_THRESHOLD) {
      issues.push({
        ruleId: rule.id, selector: rule.selector, property: decl.property,
        currentValue: decl.value, severity: 'warning', category: 'spacing',
...
export const responsiveAnalyzer: AnalysisPass = {
  ...
  analyze(ir: StyleIR, context: AnalysisContext = {} as any): AnalysisResult {
    const configBreakpoints = (context as any)?.breakpoints || { sm: '640px', md: '768px', lg: '1024px', xl: '1280px' };
    ...
    for (const rule of ir.rules) {
      ...
      if (issues.length > 0) {
        annotations.push({
          nodeId: rule.id, type: 'responsive-issues',
