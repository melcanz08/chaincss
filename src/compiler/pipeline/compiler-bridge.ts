...
import { findDirtyRules, filterDirtyIR, type IncrementalChange } from './incremental-compiler.js';
...
export async function compileWithGraphCache(
  ...
  const compoundCacheKey = crypto.createHash('sha256').update(rawKey).digest('hex');
