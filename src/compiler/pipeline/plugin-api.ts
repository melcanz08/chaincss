...
// Public plugin API v12.9 — stable
// ============================================================================

import semver from 'semver';
import type { StyleIR } from './ir/types.js';
import type { GraphService, SymbolService, DiagnosticsService, CacheService, LoggerService } from './service-registry.js';
import type {
  NormalizationResult, ValidationResult, AnalysisResult, OptimizationResult, LoweringResult,
...
export async function executeTimedAsync<T>(fn: () => Promise<T>): Promise<TimedExecution<T>> {
  const start = performance.now();
...
export type PluginPhase = 'normalize' | 'validate' | 'analyze' | 'optimize' | 'lower';
export type LifecycleHook = 'buildStart' | 'buildEnd' | 'watchStart' | 'watchEnd';
...
function getPhaseHandler<K extends PluginPhase>(plugin: ChainCSSPlugin, phase: K): PhaseHandlerMap[K] | undefined {
...
export type PluginEventType =
  | 'activated' | 'degraded' | 'failed' | 'removed'
  | 'ruleAdded' | 'ruleRemoved' | 'diagnostic' | 'cacheHit';
...
export type PluginFactory<TOptions = Record<string, unknown>> = (options?: TOptions) => ChainCSSPlugin<TOptions>;
...
const VALID_TRANSITIONS: Record<PluginState, PluginState[]> = {
  [PluginState.Registered]: [PluginState.Initialized, PluginState.Failed, PluginState.Removed],
  [PluginState.Initialized]: [PluginState.Active, PluginState.Failed, PluginState.Removed],
  [PluginState.Active]: [PluginState.Degraded, PluginState.Failed, PluginState.Removed],
  [PluginState.Degraded]: [PluginState.Active, PluginState.Failed, PluginState.Removed],
...
export class PluginRegistry {
  ...
  private executionLog = new RingBuffer<PluginExecutionContext>(MAX_EXECUTION_LOG);
  private devMode: boolean;
  private activeFeatures = new Set<string>();
  private capabilityProviders = new Map<string, Array<{ plugin: string; version: string }>>();
  private executionOrder: string[] = [];
  private eventHandlers: PluginEventHandler[] = [];

  constructor(services: PluginServices, setup: PluginSetupContext) {
    if (!services.logger) throw new Error('PluginRegistry: "logger" service is required.');
    if (!services.graph) throw new Error('PluginRegistry: "graph" service is required.');
    if (!services.diagnostics) throw new Error('PluginRegistry: "diagnostics" service is required.');
  ...
  private emit(event: PluginEvent): void {
    for (const h of this.eventHandlers) {
      try {
        ...
        this.services.logger.warn(`Plugin event handler error: ${(err as Error).message}`);
      }
    }
  }
  ...
  async use<TOptions>(factory: PluginFactory<TOptions>, options: TOptions): Promise<void>;
  async use<TOptions extends Record<string, unknown> = Record<string, unknown>>(
    ...
    if (!semver.satisfies(CURRENT_API_VERSION, plugin.meta.apiVersion)) {
      throw new Error(`Plugin "${plugin.name}" requires API ${plugin.meta.apiVersion}.`);
    }
  ...
  private setState(desc: PluginDescriptor, to: PluginState, reason?: string): void {
    ...
    if (eventType) {
      this.emit({ type: eventType, plugin: desc.plugin.name, timestamp: Date.now(), reason });
    }
  }
    ...
    for (const [, desc] of this.plugins) {
      if (desc.plugin.dependencies) {
        for (const dep of desc.plugin.dependencies) {
          if (this.plugins.has(dep)) edges.push({ from: dep, to: desc.plugin.name, type: 'dependency' });
        }
      }
      if (desc.plugin.optionalDependencies) {
        for (const dep of desc.plugin.optionalDependencies) {
          if (this.plugins.has(dep)) edges.push({ from: dep, to: desc.plugin.name, type: 'optional' });
        }
      }
      if (desc.plugin.before) {
        for (const b of desc.plugin.before) {
          if (this.plugins.has(b)) edges.push({ from: desc.plugin.name, to: b, type: 'before' });
        }
      }
      if (desc.plugin.after) {
        for (const a of desc.plugin.after) {
          if (this.plugins.has(a)) edges.push({ from: a, to: desc.plugin.name, type: 'after' });
        }
      }
    }
  ...
  private topologicalSort(plugins: ChainCSSPlugin[]): ChainCSSPlugin[] {
    ...
    for (const p of plugins) {
      if (p.dependencies) for (const dep of p.dependencies) { if (pluginMap.has(dep)) addEdge(dep, p.name, dependents, inDegree); }
      if (p.optionalDependencies) for (const dep of p.optionalDependencies) { if (pluginMap.has(dep)) addEdge(dep, p.name, dependents, inDegree); }
      if (p.after) for (const a of p.after) { if (pluginMap.has(a)) addEdge(a, p.name, dependents, inDegree); }
      if (p.before) for (const b of p.before) { if (pluginMap.has(b)) addEdge(p.name, b, dependents, inDegree); }
    }
    ...
    queue.sort((a, b) => (pluginMap.get(a)?.priority || 100) - (pluginMap.get(b)?.priority || 100));
    ...
    while (queue.length > 0) {
      ...
      for (const dep of dependents.get(name) || []) {
        ...
        if (d === 0) {
          ...
          let i = 0;
          while (i < queue.length && (pluginMap.get(queue[i])?.priority || 100) <= prio) i++;
  ...
  private async validateAndSetup(desc: PluginDescriptor): Promise<void> {
    ...
    if (plugin.meta?.requires && !semver.satisfies(this.setupDefaults.version, plugin.meta.requires)) {
      throw new Error(`Plugin "${plugin.name}" requires ChainCSS ${plugin.meta.requires}.`);
    }

    try {
      if (plugin.setup) {
        await plugin.setup({ ...this.setupDefaults, pluginOptions: plugin.options || {} });
      }
      ...
      if (plugin.provides) {
        for (const p of plugin.provides) {
          ...
          if (!this.capabilityProviders.has(name)) this.capabilityProviders.set(name, []);
          this.capabilityProviders.get(name)!.push({ plugin: plugin.name, version });
        }
      }
  ...
  async executePhase<K extends PluginPhase>(
    ...
    phase: K,
    ctx: PluginContext
  ): Promise<{ result?: Awaited<ReturnType<PhaseHandlerMap[K]>>; execution: PluginExecutionContext }> {
    const desc = this.plugins.get(plugin.name);
    if (!desc || (desc.state !== PluginState.Initialized && desc.state !== PluginState.Active && desc.state !== PluginState.Degraded)) {
      throw new Error(`Plugin "${plugin.name}" is not active.`);
    }

    const fn = getPhaseHandler(plugin, phase);
    if (!fn) {
      return { execution: { plugin: plugin.name, phase, duration: 0, success: true } };
    }

    if (ctx.ir && !Object.isFrozen(ctx.ir)) Object.freeze(ctx.ir);
    if (this.devMode) deepFreeze(ctx.ir);

    const { result, duration } = await executeTimedAsync(() => Promise.resolve(fn(ctx)));
    desc.executionTime += duration;
    desc.executionCount++;
    desc.lastExecution = Date.now();
    desc.phaseTimes[phase] = (desc.phaseTimes[phase] || 0) + duration;

    const execution: PluginExecutionContext = { plugin: plugin.name, phase, duration, success: true };
  ...
  async executeHook(hook: LifecycleHook, ctx: PluginContext, options?: LifecycleOptions): Promise<void> {
    const policy = { ...DEFAULT_LIFECYCLE, ...options };
    const hookPolicy = policy[hook as keyof LifecycleOptions] || 'continue';
    const activeByName = new Map(this.getActive().map(d => [d.plugin.name, d]));
    ...
    for (const name of this.executionOrder) {
      ...
      try {
        ...
        this.executionLog.push({ plugin: desc.plugin.name, phase: hook, duration, success: true });
      } catch (e) {
  ...
  async executeBeforeEmit(ctx: PluginContext): Promise<void> {
    ...
    for (const name of this.executionOrder) {
      ...
      try {
        const { duration } = await executeTimedAsync(() => Promise.resolve(fn(ctx)));
        desc.executionCount++;
        desc.lastExecution = Date.now();
        desc.phaseTimes['beforeEmit'] = (desc.phaseTimes['beforeEmit'] || 0) + duration;
        this.executionLog.push({ plugin: desc.plugin.name, phase: 'beforeEmit', duration, success: true });
      } catch (e) {
        const err = e as Error;
        desc.failureCount++;
        desc.lastFailure = Date.now();
  ...
  async executeAfterEmit(ctx: PluginContext, css: string): Promise<string> {
    ...
    for (const name of this.executionOrder) {
      ...
      try {
        const timed = await executeTimedAsync(() => Promise.resolve(fn(ctx, result)));
        result = timed.result;
        desc.executionCount++;
        desc.lastExecution = Date.now();
        desc.phaseTimes['afterEmit'] = (desc.phaseTimes['afterEmit'] || 0) + timed.duration;
        this.executionLog.push({
          plugin: desc.plugin.name,
          phase: 'afterEmit',
  ...
  getReady(phase: PluginPhase): ChainCSSPlugin[] {
    ...
    return this.executionOrder
      ...
      .map(d => d.plugin)
      .filter(p => p[phase] && (!p.supports?.length || p.supports.some(f => this.activeFeatures.has(f))));
  }
  ...
  validateCapabilities(): CapabilityIssue[] {
    ...
    const activePluginNames = new Set(this.getActive().map(d => d.plugin.name));
    ...
    for (const desc of this.getActive()) {
      ...
      for (const entry of plugin.consumes) {
        ...
        const available = (this.capabilityProviders.get(c) || []).filter(p => activePluginNames.has(p.plugin));

        if (available.length === 0) {
          issues.push({
            plugin: plugin.name, capability: c,
   ...
   */
  async validateCapabilityIncremental(capability: string): Promise<CapabilityIssue[]> {
    const issues: CapabilityIssue[] = [];
    const activePluginNames = new Set(this.getActive().map(d => d.plugin.name));
  ...
  async remove(name: string): Promise<void> {
    for (const [, desc] of this.plugins) {
      if (desc.plugin.dependencies?.includes(name)) {
        throw new Error(`Cannot remove "${name}": plugin "${desc.plugin.name}" depends on it.`);
      }
    }

    for (const [, desc] of this.plugins) {
      if (desc.plugin.optionalDependencies?.includes(name) && desc.state !== PluginState.Removed) {
        this.setState(desc, PluginState.Degraded, `Optional dependency "${name}" removed.`);
      }
    }
  ...
  get(name: string): ChainCSSPlugin | undefined { return this.plugins.get(name)?.plugin; }
  getDescriptor(name: string): PluginDescriptor | undefined { return this.plugins.get(name); }
  getState(name: string): PluginState | undefined { return this.plugins.get(name)?.state; }
  list(): string[] { return Array.from(this.plugins.keys()); }

  getActive(): PluginDescriptor[] {
    return Array.from(this.plugins.values()).filter(
      d => d.state === PluginState.Initialized || d.state === PluginState.Active || d.state === PluginState.Degraded
    );
  }

  getActivePlugins(): ChainCSSPlugin[] { return this.getActive().map(d => d.plugin); }

  getFailed(): PluginDescriptor[] {
    return Array.from(this.plugins.values()).filter(d => d.state === PluginState.Failed);
  }

  getFailedPlugins(): ChainCSSPlugin[] { return this.getFailed().map(d => d.plugin); }
  getAll(): PluginDescriptor[] { return Array.from(this.plugins.values()); }
  getAllPlugins(): ChainCSSPlugin[] { return this.getAll().map(d => d.plugin); }
}
...
export function createPluginRegistry(services: PluginServices, setup: PluginSetupContext): PluginRegistry {
