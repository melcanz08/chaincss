// src/compiler/pipeline/normalizers/scroll-timeline.ts
// Merges your old full engine (presets, compileScrollAnimation) with native v2.12 entangle() shorthand
// Supports both:
//   chain().entangle('scroll', { y: '-10% -> 10%' })  -> new shorthand
//   compileScrollAnimation({ preset: 'parallax' })    -> old full API (still works)
...
// --- Presets from your old file (kept 100%) ---
...
// --- Core Compiler (upgraded to emit modern syntax + @supports) ---
let animCounter = 0;
...
export function compileScrollAnimation(animation: ScrollAnimation): ScrollTimelineResult {
  ...
  if (animation.timeline.source === 'view') css += `  animation-range: ${animation.timeline.range || 'entry 0% cover 50%'};\n`;
...
export function compileScrollAnimations(animations: ScrollAnimation[]): string {
  let css = `/* ChainCSS Scroll-Driven Animations - ${new Date().toISOString()} */\n\n`;
...
export function createScrollAnimation(preset: keyof typeof SCROLL_PRESETS, selector: string, overrides?: Partial<ScrollAnimation>): ScrollAnimation {
  const base = SCROLL_PRESETS[preset]; if (!base) throw new Error('Unknown scroll preset: ' + preset);
...
// --- NEW v2.12: Shorthand entangle API ---
...
function parseRange(v: string){ if(!v?.includes('->')) return {from:v,to:v}; const [f,t]=v.split('->').map(s=>s.trim()); return {from:f,to:t}; }
...
export function createScrollTimeline(selector: string, opts: ScrollEntangleOptions) {
  ...
  if(opts.y){ const {from,to}=parseRange(opts.y); frames['0%'].transform=`${frames['0%'].transform||''} translateY(${from})`.trim(); frames['100%'].transform=`${frames['100%'].transform||''} translateY(${to})`.trim(); }
  if(opts.x){ const {from,to}=parseRange(opts.x); frames['0%'].transform=`${frames['0%'].transform||''} translateX(${from})`.trim(); frames['100%'].transform=`${frames['100%'].transform||''} translateX(${to})`.trim(); }
  if(opts.scale){ const {from,to}=parseRange(opts.scale); frames['0%'].transform=`${frames['0%'].transform||''} scale(${from})`.trim(); frames['100%'].transform=`${frames['100%'].transform||''} scale(${to})`.trim(); }
  if(opts.opacity){ const {from,to}=parseRange(opts.opacity); frames['0%'].opacity=from; frames['100%'].opacity=to; }
...
export function scrollEntangleMacro(value: any, ctx: any){
  ...
  if(!ctx._entangle) ctx._entangle=[]; ctx._entangle.push({type:'scroll',opts:value,native:true});
  ctx.willChange='transform, opacity'; ctx.transform=ctx.transform||'translateZ(0)';
}
