...
export const atomicExtractor: OptimizationPass = {
  ...
  optimize(ir: StyleIR, context?: any): OptimizationResult {
    ...
    const usageMap = new Map<string, { count: number; property: string; value: string | number; pseudo: string; media: string }>();
    ...
    for (const [key, data] of usageMap) {
      ...
      const RESERVED = new Set([
        ...
        'hidden', 'none', 'static', 'relative', 'absolute', 'fixed', 'sticky'
      ]);

      const rawName = generateAtomicClassName(data.property, data.value, data.pseudo, data.media, context);
    ...
    for (const rule of ir.rules) {
      ...
      rule.declarations = rule.declarations.filter((decl: any) => {
        ...
        if (className) {
          ...
          try { 
            recordHistory(decl as any, 'atomic-extractor', 'extracted-to-atomic', key, `Moved to atomic class .${className}`); 
          } catch {}
          return false;
        }
        return true;
      ...
      if (atomicClasses.length > 0) {
        (rule.meta as any).atomicClasses = [...((rule.meta as any).atomicClasses || []), ...atomicClasses];
        (rule.meta as any).atomicCount = ((rule.meta as any).atomicClasses?.length ?? 0);
    ...
    return { 
      ...
      savings: { rulesEliminated: 0, declarationsEliminated: declarationsReplaced, bytesSaved }, 
      changes: atomicRules.length + declarationsReplaced 
    };
  },
};

function generateAtomicClassName(property: string, value: string | number, pseudo: string, media: string, context?: any): string {
  const customShorthands = context?.shorthands || context?.config?.shorthands || {};
  ...
  const abbreviations: Record<string, string> = {
    ...
    'cursor': 'cursor-', 'overflow': 'overflow-', 'text-align': 'text-', 'justify-content': 'justify-', 'align-items': 'items-', 'gap': 'gap-', 'box-shadow': 'shadow-', 'transition': 'transition-', 'flex-direction': 'flex-', ...customShorthands,
  };
  ...
  let name = (property === 'display' || property === 'position') ? cleanValue : prefix + cleanValue;
