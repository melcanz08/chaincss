...
function resolveAllTokens(
  ...
  const matches = [
    ...value.matchAll(/\$([a-zA-Z0-9_-]+(?:\.[a-zA-Z0-9_-]+)*)/g),
  ];
      ...
      for (const decl of rule.declarations || []) {
        if (typeof decl.value === "string" && decl.value.includes("$")) {
          ...
          if (options.inlineLiterals) {
            const { value: resolved, hadUnresolved } = resolveAllTokens(
              originalRawValue,
              resolver,
              options.activeTheme,
            );
          ...
          const fallback = originalRawValue.replace(
            ...
            (match: string, rawPath: string) => {
              const checked = resolveTokenPath(
                match,
                resolver,
                options.activeTheme,
              );
          ...
          const checkUnresolved = resolveAllTokens(
            originalRawValue,
            resolver,
            options.activeTheme,
          );
