...
export function parseCSSValue(input: string): CSSValueNode {
  ...
  // Try numeric values
  const dimensionMatch = trimmed.match(/^(-?\d+(?:\.\d+)?)(px|em|rem|vh|vw|%|ms|s|deg|rad|ch|ex|cm|mm|in|pt|pc|dpi|dpcm|dppx)$/i);
  if (dimensionMatch) {
...
function parseVariable(input: string): VariableNode {
  ...
  const fallback = parts.length > 1 ? parseCSSValue(parts.slice(1).join(',').trim()) : undefined;
...
export function optimizeAST(node: CSSValueNode): CSSValueNode {
  switch (node.kind) {
    case 'binary': {
      ...
      if (right.kind === 'number' && right.value === 0 && (node.operator === '+' || node.operator === '-')) {
        return left;
      }
      if (right.kind === 'number' && right.value === 1 && node.operator === '*') {
        return left;
      }
      if (right.kind === 'number' && right.value === 1 && node.operator === '/') {
        return left;
      ...
      if (left.kind === 'number' && left.value === 0 && (node.operator === '*' || node.operator === '/')) {
...
function evaluateBinary(a: number, b: number, op: BinaryOperator): number | null {
  switch (op) {
    case '+': return a + b;
    case '-': return a - b;
    case '*': return a * b;
...
export function fromParsedValue(parsed: any): CSSValueNode {
  if (!parsed || !parsed.kind) return { kind: 'keyword', value: String(parsed) };
  ...
  switch (parsed.kind) {
    ...
    case 'function':
      return { kind: 'function', name: parsed.name, args: (parsed.args || []).map(fromParsedValue) };
    case 'list':
      return { kind: 'list', items: (parsed.items || []).map(fromParsedValue), separator: ' ' };
    case 'raw':
