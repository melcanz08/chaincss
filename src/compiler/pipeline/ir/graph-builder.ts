...
import type { StyleIR, IRRule, IRNodeId, IRGraph, IRGraphEdge, IRDeclaration, IRAtRule } from './types.js';
...
export function buildIRGraph(ir: StyleIR): IRGraph {
  ...
  function collectRules(rulesInput: IRRule[] | Record<string, IRRule> | undefined | null, parentId?: IRNodeId, containerAtRule?: IRAtRule) {
  ...
  const nodeData = nodeArray.map(rule => {
    const selectorParts = new Set((rule.selector || '').split(/[\s>+~,]+/).filter(Boolean));
  ...
  for (const dataA of nodeData) {
    ...
    for (const ruleB of candidates) {
      const pairKey = dataA.rule.id < ruleB.id ? `${dataA.rule.id}:${ruleB.id}` : `${ruleB.id}:${dataA.rule.id}`;
...
function addEdge(
  ...
  const exists = edges.some(e => e.from === from.id && e.to === to.id && e.type === type);
  if (exists) return;
...
function extractTokenReferences(rule: IRRule): Set<string> {
  ...
  function scanDeclarations(declsInput: IRDeclaration[] | Record<string, IRDeclaration> | undefined | null) {
...
function findAtRuleById(ir: StyleIR, id: string): IRAtRule | null {
  function search(rulesInput: IRRule[] | Record<string, IRRule> | undefined | null): IRAtRule | null {
    const rules = ensureArray(rulesInput);
    for (const rule of rules) {
      if (!rule || typeof rule !== 'object') continue;
      const atRules = ensureArray(rule.atRules);
...
function detectTokenDerivationEdges(
  ...
  const tokenRelationships = ensureArray(irMeta?._tokenRelationships || irMeta?.tokenRelationships);

  for (const rawRel of tokenRelationships) {
    const rel = rawRel as { type?: string; source?: string; target?: string; method?: string };
...
export function traverseGraph(
  ...
  const queue: Array<{ id: IRNodeId; depth: number }> = ensureArray(startNodes).map(id => ({ id, depth: 0 }));
...
export function findAffectedNodes(graph: IRGraph, changedNodeId: IRNodeId): IRNodeId[] {
...
export function getGraphStats(graph: IRGraph) {
  ...
  return {
    ...
    averageDependencies: allNodes.reduce((sum, n) => sum + ensureArray(n.meta?.dependencies).length, 0) / (allNodes.length || 1),
    averageDependents: allNodes.reduce((sum, n) => sum + ensureArray(n.meta?.dependents).length, 0) / (allNodes.length || 1),
  };
}
...
function findMaxDepth(graph: IRGraph): number {
  ...
  if (graph?.nodes) {
    const nodeKeys = graph.nodes instanceof Map 
  ...
  if (graph?.nodes) {
    for (const [id, rule] of getNodeEntries(graph)) {
      ...
      nodes.push({
        ...
        atRuleType: isAtRule ? (ensureArray(graph.edges).find(e => e.to === id)?.metadata?.atRuleType as string) : undefined,
        atRuleQuery: isAtRule ? (ensureArray(graph.edges).find(e => e.to === id)?.metadata?.query as string) : undefined,
      });
    }
  }
