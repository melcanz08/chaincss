...
import type { PipelineStageResult, Diagnostic, Correction } from './pipeline-types.js';
...
export function generateDiagnosticsReport(
  ...
  const allDeclarations = allRules.reduce(
    (sum, r) => sum + r.declarations.length + r.pseudoClasses.reduce((s, pc) => s + pc.declarations.length, 0),
    0
  );
  ...
  return {
    ...
    performance: {
      ...
      averagePassDuration: timeline.length > 0 ? Math.round(totalPassTime / timeline.length * 100) / 100 : 0,
    },
  };
}
...
export function formatDiagnosticsReport(report: DiagnosticsReport): string {
  ...
  lines.push(`  Slowest pass: ${report.performance.slowestPass} (${report.performance.slowestPassDuration.toFixed(2)}ms)`);
  lines.push(`  Avg pass: ${report.performance.averagePassDuration.toFixed(2)}ms`);
  lines.push('');
  lines.push('  IR Statistics:');
  lines.push(`    Rules: ${report.irStats.totalRules} (${report.irStats.deadRules} dead)`);
  ...
  if (report.diagnostics.items.length > 0) {
    ...
    for (const item of report.diagnostics.items) {
      const icon = item.severity === 'error' ? '❌' : item.severity === 'warning' ? '⚠️' : item.severity === 'info' ? 'ℹ️' : '💡';
  ...
  if (report.corrections.length > 0) {
    ...
    for (const c of report.corrections) {
      lines.push(`    🔧 [${c.pass}] ${c.property || '?'}: ${c.original} → ${c.corrected}`);
      lines.push(`       ${c.reason}`);
    }
    lines.push('');
  }
  ...
  for (const pt of report.passTimings) {
    ...
    lines.push(`    ${pt.pass.padEnd(30)} ${pt.duration.toFixed(2).padStart(6)}ms ${bar}`);
  }
  lines.push('');
  lines.push('═'.repeat(60));
