...
export function getAffectedDeclarations(
  ...
  for (const d of rule.declarations || []) {
    ...
    const calculatedAfter =
      nextEvent?.previous !== undefined && nextEvent?.previous !== null
        ? String(nextEvent.previous)
        : String(d.value);

    const beforeVal =
      firstEvent?.previous !== undefined && firstEvent?.previous !== null
        ? String(firstEvent.previous)
        : calculatedAfter;

    // skip no-ops early (saves snapshot dedup work)
    if (
      beforeVal === calculatedAfter &&
      passEvents.length === 1 &&
      firstEvent?.previous === undefined
    ) {
