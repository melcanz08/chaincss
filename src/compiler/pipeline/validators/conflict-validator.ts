...
export const conflictValidator: ValidationPass = {
  ...
  validate(ir: StyleIR): ValidationResult {
    ...
    for (const rule of ir.rules) {
      ...
      for (const d of (rule.declarations || [])) {
        if (d.property && d.value !== undefined && d.value !== null) {
          ...
          declMap.set(kebabProp, String(d.value).replace(/!important/g, '').trim());
        }
      }
      ...
      const parentFlexProps = ['justify-content', 'align-items', 'flex-direction', 'flex-wrap', 'align-content', 'justify-items'];
      const hasParentFlexProps = parentFlexProps.some(prop => declMap.has(prop));
      if (hasParentFlexProps && display !== 'flex' && display !== 'inline-flex') {
        diagnostics.push({
      ...
      const parentGridProps = ['grid-template-columns', 'grid-template-rows', 'grid-template-areas', 'grid-auto-flow', 'grid-auto-columns', 'grid-auto-rows'];
      const hasParentGridProps = parentGridProps.some(prop => declMap.has(prop));
      if (hasParentGridProps && display !== 'grid' && display !== 'inline-grid') {
        diagnostics.push({
      ...
      if (display === 'inline' && position && ['absolute', 'fixed'].includes(position)) {
        diagnostics.push({
    ...
    return { diagnostics, passed: errors === 0, stats: { errors, warnings, info, hints } };
  },
};
