...
async function safeImport(name: string): Promise<any> {
  try { const fn = new Function('name', 'return import(name)'); return await fn(name); } catch { return null; }
}
...
async function loadPostcss() {
  ...
  try { const m = await safeImport('postcss'); postcss = m?.default || m; } catch {}
  postcssLoaded = true; return postcss;
}
async function loadAutoprefixer() {
  if (autoprefixerLoaded) return autoprefixer;
  try { const m = await safeImport('autoprefixer'); autoprefixer = m?.default || m; } catch {}
  autoprefixerLoaded = true; return autoprefixer;
}
...
export interface PrefixerResult { css: string; map: string | null; warnings?: string[]; }
export interface ProcessOptionsWithPaths { from?: string; to?: string; map?: boolean | object; }
...
export class ChainCSSPrefixer {
  ...
  async determineMode(): Promise<'full' | 'lightweight'> {
    if (this.config.mode === 'full') {
      ...
      if (!has && this.config.verbose) console.warn('[ChainCSS] Full mode needs postcss + autoprefixer. Run: npm i -D postcss autoprefixer — falling back to lightweight');
      return has ? 'full' : 'lightweight';
    }
    if (this.config.mode === 'auto' && this.config.verbose) console.log('[ChainCSS] mode:auto is deprecated, using lightweight. Set mode:lightweight or mode:full');
    return 'lightweight';
  }
  async process(cssString: string, options: ProcessOptionsWithPaths = {}): Promise<PrefixerResult> {
    this.warnings = [];
    if (!this.config.enabled) return { css: cssString, map: null, warnings: [] };
    try {
      const mode = await this.determineMode();
      if (mode === 'full') return await this.processWithAutoprefixer(cssString, options);
  ...
  private async processWithAutoprefixer(cssString: string, options: ProcessOptionsWithPaths): Promise<PrefixerResult> {
    const ap = await loadAutoprefixer(); const pc = await loadPostcss();
    if (!ap || !pc) return this.processWithBuiltIn(cssString);
    const result = await pc([ap({
      overrideBrowserslist: this.config.browsers,
      remove: this.config.remove, add: this.config.add,
      flexbox: this.config.flexbox, grid: this.config.grid
    })]).process(cssString, {
      from: options.from || 'input.css', to: options.to || 'output.css',
      map: this.config.sourceMap ? { inline: this.config.sourceMapInline, annotation: false } : false
    });
    return { css: result.css, map: result.map ? result.map.toString() : null, warnings: this.warnings };
  }
  private processWithBuiltIn(cssString: string): PrefixerResult {
    return { css: this.lightweightPrefix(cssString), map: null, warnings: this.warnings };
  }
  private lightweightPrefix(cssString: string): string {
    const comments: string[] = [];
    let cleanCss = cssString.replace(COMMENT_REGEX, (match) => {
      comments.push(match); return `/*__COMMENT_PLACEHOLDER_${comments.length - 1}__*/`;
    });
    cleanCss = this.duplicateKeyframes(cleanCss);
    let result = ''; let depth = 0; let inString: string | null = null; let currentBlock = ''; let selectorOrAtRule = '';
    for (let i = 0; i < cleanCss.length; i++) {
      const char = cleanCss[i];
      if ((char === '"' || char === "'") && cleanCss[i - 1] !== '\\') {
        if (inString === char) inString = null; else if (!inString) inString = char;
      }
      if (inString) { if (depth > 0) currentBlock += char; else selectorOrAtRule += char; continue; }
      if (char === '{') { depth++; if (depth === 1) { result += selectorOrAtRule + '{'; selectorOrAtRule = ''; currentBlock = ''; continue; } }
      else if (char === '}') { depth--; if (depth === 0) { result += this.prefixDeclarationBlock(currentBlock) + '}'; currentBlock = ''; continue; } }
      if (depth > 0) currentBlock += char; else selectorOrAtRule += char;
    }
    result += selectorOrAtRule;
    return result.replace(/\/\*__COMMENT_PLACEHOLDER_(\d+)__\*\//g, (_, id) => comments[parseInt(id, 10)] || '');
  }
  private duplicateKeyframes(css: string): string {
    let index = 0; const keyword = '@keyframes';
    while (true) {
      const startIdx = css.indexOf(keyword, index); if (startIdx === -1) break;
      const openBraceIdx = css.indexOf('{', startIdx);
      if (openBraceIdx === -1) { index = startIdx + keyword.length; continue; }
      let braceCount = 1; let scanIdx = openBraceIdx + 1;
      while (scanIdx < css.length && braceCount > 0) {
        const ch = css[scanIdx]; if (ch === '{') braceCount++; else if (ch === '}') braceCount--; scanIdx++;
      }
      if (braceCount === 0) {
        ...
        if (css.includes(`@-webkit-keyframes${namePart}`) || css.includes(`@-webkit-keyframes ${namePart}`)) { index = startIdx + keyword.length; continue; }
        const replacement = `${webkitBlock}\n${fullBlock}`;
        css = css.slice(0, startIdx) + replacement + css.slice(scanIdx);
        index = startIdx + replacement.length;
      } else { index = startIdx + keyword.length; }
    }
    return css;
  }
  private prefixDeclarationBlock(block: string): string {
    return block.replace(DECL_REGEX, (full, prop, value, semi) => {
      const trimmedProp = prop.trim(); const trimmedVal = value.trim();
      const map = LIGHTWEIGHT_PREFIX_MAP[trimmedProp]; const vMap = LIGHTWEIGHT_VALUE_PREFIXES[trimmedProp];
