...
export function showMetrics(force: boolean = false): void {
  ...
  const hasData =
    Object.values(snapshot.counters).some((v) => v > 0) ||
  ...
  // Counters
  const counterEntries = Object.entries(snapshot.counters).filter(
    ([, v]) => v > 0,
  );
  ...
  console.log(
    `    Pipeline Passes: ${snapshot.counters.pipeline_passes_run || 0}`,
  );
