...
import type { ChainCSSConfig, ChainCSSUserConfig } from '@shared/config/index.js'
...
function normalize(mod: any): ChainCSSUserConfig { 
  return (mod?.default ?? mod) as ChainCSSUserConfig || {} as ChainCSSUserConfig 
}
 ...
 */
async function mergePresets(base: ChainCSSUserConfig): Promise<ChainCSSUserConfig> {
...
export async function loadConfig(configPath?: string): Promise<ChainCSSConfig> {
  ...
  for (const configFile of possiblePaths) {
    ...
    try {
      ...
      logger.warn(`Failed to parse config element boundary at ${configFile}:`, error)
    }
  }
