...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (fs.existsSync(configPath) && !options.force) {
    logger.warn(
      `Configuration asset [${configFilename}] already exists. Run with --force to overwrite.`,
    );
    return;
  }
  ...
  const finalStylePath = path.join(
    srcDirectory,
    `styles.chain.${styleExtension}`,
  );
  ...
  if (!fs.existsSync(finalStylePath) || options.force) {
    ...
    logger.info(
      `Styling framework rules already established at: ${finalStylePath}`,
    );
  }
  ...
  logger.info("\n📚 Next Steps:");
  logger.info(
    `  1. Open and adjust parameters within ${chalk.cyan(configFilename)}`,
  );
  logger.info(
    `  2. Add utility declaration blocks inside ${chalk.cyan(path.relative(process.cwd(), finalStylePath))}`,
  );
  logger.info(
    `  3. Execute production asset build passes: ${chalk.cyan("npx chaincss build")}`,
  );
  logger.info(
    `  4. Launch live directory development loops: ${chalk.cyan("npx chaincss watch")}\n`,
  );
}
