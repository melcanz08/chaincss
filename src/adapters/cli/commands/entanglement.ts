...
function loadTokens(
  filePath: string,
  isFigma?: boolean,
): { raw: any; flat: Record<string, any> } {
  ...
  const flat = isFigmaFormat
    ? importFigmaTokens(raw)
    : raw.tokens || raw.values || raw;
...
function detectChanges(oldTokens: any, newTokens: any, prefix = ""): string[] {
  ...
  for (const k of keys) {
    ...
    if (ovIsObj && nvIsObj) {
      const isTokenNode =
        ("value" in ov || "$value" in ov) && ("value" in nv || "$value" in nv);
      if (isTokenNode) {
...
export async function entanglementCommand(opts: EntanglementOptions = {}) {
  ...
  if (!inputPath) {
    logger.error(
      "[entanglement] No tokens file found. Use --input tokens.json",
    );
    process.exit(1);
  }
  ...
  let relationships: Relationship[] =
    config.tokens?.relationships || config.entanglement?.relationships || [];
  if (!relationships.length) {
    logger.warn(
      "[entanglement] No relationships found in chaincss.config.ts -> tokens.relationships.",
    );
  }
    ...
    try {
      ...
      if (!lastTokens) {
        ...
        if (report.changes.length) {
          logger.success(
            `[entanglement] Fixed ${report.changes.length} tokens`,
          );
          ...
          logger.success(
            `[entanglement] Wrote ${path.relative(root, outputPath)}`,
          );
        } else {
      ...
      logger.info(
        `[entanglement] Detected change in ${changedPaths.join(", ")}${changedFile ? ` (${path.basename(changedFile)})` : ""}`,
      );
      ...
      for (const changedPath of changedPaths) {
        ...
        const rep = engine.propagate(
          workingTokens,
          changedPath,
          String(newVal),
          { autoFixContrast: opts.fix !== false },
        );
      ...
      if (allChanges.length) {
        ...
        logger.success(
          `[entanglement] Propagated ${allChanges.length} changes -> ${path.relative(root, outputPath)}`,
        );
      }
  ...
  logger.info(
    `[entanglement] Watching ${path.relative(root, inputPath)} for changes... (Ctrl+C to exit)`,
  );
  ...
  const cfgFile = fs.existsSync(configPath)
    ? configPath
  ...
  if (cfgFile) {
    fs.watch(cfgFile, () => {
      ...
      loadConfig()
        .then((c: any) => {
          relationships =
            c.tokens?.relationships || c.entanglement?.relationships || [];
          ...
          logger.success(
            `[entanglement] Re-initialized with ${relationships.length} relationships`,
          );
        })
        .catch((e) => {
