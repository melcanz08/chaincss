...
export async function cacheCommand(action: string, options: any) {
  const cacheDir = path.resolve(
    process.cwd(),
    options.cacheDir || "./.chaincss-cache",
  );
  const persistentCacheDir = path.resolve(
    process.cwd(),
    options.persistentCacheDir || "./.chaincss/persistent-cache",
  );
  ...
  switch (action) {
    case "clear":
      ...
      try {
        ...
        console.log(
          chalk.gray(`    Removed: ${path.relative(process.cwd(), cacheDir)}`),
        );
        console.log(
          chalk.gray(
            `    Removed: ${path.relative(process.cwd(), persistentCacheDir)}\n`,
          ),
        );
      } catch (error) {
        console.log(
    ...
    case "stats":
      try {
        ...
        console.log(
          `  Status: ${persistentCacheExists ? chalk.green("Active") : chalk.gray("Empty")}`,
        );
        if (persistentCacheExists) {
          console.log(`  Entry count: ${chalk.white(stats.entryCount || 0)}`);
          console.log(
            `  Total size: ${chalk.white((stats.totalSizeMB || 0).toFixed(2))} MB`,
          );
          console.log(
            `  Total size (bytes): ${chalk.white(formatBytes(stats.totalSizeBytes || 0))}`,
          );

          if (stats.oldestEntry) {
            const age = Date.now() - new Date(stats.oldestEntry).getTime();
            console.log(
              `  Oldest entry: ${chalk.white(new Date(stats.oldestEntry).toLocaleDateString())} (${formatDuration(age)} ago)`,
            );
          }
          if (stats.newestEntry) {
            const age = Date.now() - new Date(stats.newestEntry).getTime();
            console.log(
              `  Newest entry: ${chalk.white(new Date(stats.newestEntry).toLocaleDateString())} (${formatDuration(age)} ago)`,
            );
          }
            ...
            const hitRateColor =
              stats.hitRate > 0.8
                ...
                : stats.hitRate > 0.5
                  ...
                  : chalk.red;
            console.log(
              `  Cache hit rate: ${hitRateColor(`${(stats.hitRate * 100).toFixed(1)}%`)}`,
            );
          }
        }
        ...
        console.log(
          `  Status: ${regularCacheExists ? chalk.green("Active") : chalk.gray("Empty")}`,
        );
        ...
        console.log(
          `  Cache directory: ${chalk.gray(path.relative(process.cwd(), persistentCacheDir))}`,
        );
    ...
    case "prune":
      try {
        ...
        console.log(
          chalk.gray(
            `    Before: ${beforeCount} entries, ${beforeSize.toFixed(2)} MB`,
          ),
        );
        ...
        if (removedCount > 0) {
          ...
          console.log(
            chalk.gray(
              `    Removed: ${removedCount} entries, ${removedSize.toFixed(2)} MB`,
            ),
          );
          console.log(
            chalk.gray(
              `    Remaining: ${afterCount} entries, ${afterSize.toFixed(2)} MB`,
            ),
          );
        } else {
    ...
    case "inspect":
      ...
      if (!key) {
        ...
        console.log(
          chalk.gray("    Usage: chaincss cache inspect --key <key>\n"),
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `  Created: ${chalk.gray(entry.createdAt ? new Date(entry.createdAt).toLocaleString() : "Unknown")}`,
        );
        console.log(
          `  Last accessed: ${chalk.gray(entry.lastAccessed ? new Date(entry.lastAccessed).toLocaleString() : "Never")}`,
        );
    ...
    case "delete":
      ...
      if (!deleteKey) {
        ...
        console.log(
          chalk.gray("    Usage: chaincss cache delete --key <key>\n"),
        );
        return;
      }
    ...
    case "validate":
      try {
        ...
        const validationResults = await poolPromises(
          entries,
          async (entry: any) => {
        ...
        console.log(
          `  Invalid entries: ${invalidCount > 0 ? chalk.red(invalidCount) : chalk.green(invalidCount)}`,
        );
    ...
    case "backup":
      const rawBackupPath =
        options.output || `./.chaincss-cache-backup-${Date.now()}`;
      ...
      try {
        ...
        const isNestedInCache =
          resolvedBackupPath.startsWith(cacheDir + path.sep) ||
          resolvedBackupPath === cacheDir;
        const isNestedInPersistent =
          resolvedBackupPath.startsWith(persistentCacheDir + path.sep) ||
          resolvedBackupPath === persistentCacheDir;

        if (isNestedInCache || isNestedInPersistent) {
          throw new Error(
            "Target backup directory cannot be nested within the cache folders being backed up.",
          );
        }
        ...
        if (fs.existsSync(persistentCacheDir)) {
          fs.cpSync(
            persistentCacheDir,
            path.join(backupCacheDir, "persistent"),
        ...
        console.log(
          chalk.green(
            `✓ Cache backed up to ${path.relative(process.cwd(), backupCacheDir)}`,
          ),
        );
        console.log(
          chalk.gray(
            `    To restore, copy the contents back to the cache directory\n`,
          ),
        );
      } catch (error) {
        console.log(
    ...
    default:
      ...
      console.log(
        chalk.cyan("  clear     ") + chalk.gray("- Clear all cache data"),
      );
      console.log(
        chalk.cyan("  stats     ") + chalk.gray("- Show cache statistics"),
      );
      console.log(
        chalk.cyan("  prune     ") + chalk.gray("- Remove expired entries"),
      );
      console.log(
        chalk.cyan("  list      ") + chalk.gray("- List all cache entries"),
      );
      console.log(
        chalk.cyan("  inspect   ") +
          chalk.gray("- Inspect a specific cache entry"),
      );
      console.log(
        chalk.cyan("  delete    ") +
          chalk.gray("- Delete a specific cache entry"),
      );
      console.log(
        chalk.cyan("  validate  ") + chalk.gray("- Validate cache integrity"),
      );
      console.log(
        chalk.cyan("  backup    ") + chalk.gray("- Backup cache to file"),
      );
  }
}
