// ============================================================================
// FILE: chaincss/src/adapters/cli/commands/audit.ts
// ============================================================================
...
function contrastFromRgb(
  ...
  const l1 = relativeLuminance(fg.r, fg.g, fg.b) + 0.05,
    l2 = relativeLuminance(bg.r, bg.g, bg.b) + 0.05;
...
export function findClosestFix(
  ...
  background: string,
  target = 4.5,
): { fixed: string; ratio: number; method: string } | null {
...
export async function auditCommand(opts: AuditOptions = {}) {
  ...
  if (tokensPath) {
    ...
    if (
      rawTokens.$themes ||
      Object.values(rawTokens).some(
        (v: any) => v && typeof v === "object" && "value" in v,
      )
    ) {
      logger.info("[audit] Figma format detected");
      tokens = importFigmaTokens(rawTokens);
    }
  }

  let contract: any = null;
  if (opts.contract) {
    const cp = path.isAbsolute(opts.contract)
      ? opts.contract
      : path.join(root, opts.contract);
    if (fs.existsSync(cp)) {
      try {
        ...
        if (cp.endsWith(".ts")) {
          ...
          logger.warn(
            `  Ensure you compile '${path.relative(root, cp)}' to JavaScript or run using a wrapper like tsx.\n`,
          );
        }
        throw importErr;
      }
  ...
  if (Array.isArray(config.a11y?.pairs)) {
    for (const p of config.a11y.pairs) {
      ...
      const fgVal =
        p.foreground?.startsWith("#") || p.foreground?.startsWith("rgb")
          ? p.foreground
          : resolve(p.foreground, tokens) || p.foreground;
      const bgVal =
        p.background?.startsWith("#") || p.background?.startsWith("rgb")
          ? p.background
          : resolve(p.background, tokens) || p.background;
      if (fgVal && bgVal)
        pairs.push({
  ...
  if (pairs.length === 0) {
    logger.warn("[audit] No pairs found. Add a11y.pairs in config.");
  ...
  if (!opts.silent) {
    ...
    for (const c of report.checks) {
      ...
      if (failing && opts.fix) {
        const pair = pairs.find(
          (p: any) =>
            p.color === c.foreground && p.backgroundColor === c.background,
        );
        ...
        if (fix) {
          ...
          logger.error(
            `✗ ${c.foreground} on ${c.background} → ${c.ratio}:1 FAIL — fix: ${fix.fixed} → ${fix.ratio}:1 (${fix.method})`,
          );
          continue;
        }
      }
      ...
      if (status === "FAIL") logger.error(line);
      else if (status === "WARN") logger.warn(line);
    ...
    if (fixes.length) {
      ...
      for (const f of fixes) {
        console.log(
          ` ${f.selector}: ${f.original} → ${f.fixed} (${f.method}, ${f.ratio}:1 on ${f.background})`,
        );
      }
      console.log("");
    }
  ...
  if (opts.json) {
    const out = path.isAbsolute(opts.json)
      ? opts.json
      : path.join(root, opts.json);
    fs.writeFileSync(
      out,
      JSON.stringify({ ...report, fixes, pairs }, null, 2),
      "utf8",
    );
  ...
  if (opts.fix && opts.write && tokensPath && fixes.length) {
    ...
    if (mutated) {
      ...
      logger.success(
        `[audit] --write applied ${fixes.length} fix(es) to ${path.relative(root, tokensPath)}`,
      );
    }
  }
  ...
  if (
    failOn === "AAA" &&
    (report.failCount > 0 || report.warnings.length > 0) &&
    !opts.fix
  ) {
    process.exit(1);
  }
  ...
  if (opts.fix && fixes.length && !opts.write) {
    logger.info(
      "[audit] Run with --write to apply fixes, or --fail-on none to not block CI while fixing",
    );
  }
