...
export async function buildCommand(options: BuildOptions): Promise<void> {
  ...
  if (isPersistent) {
    ...
    const projectHash = Buffer.from(process.cwd()).toString('base64').substring(0, 8);
    try {
      compilerState = await restoreCompilerStateFromDisk(persistentCache, projectHash);
    } catch {
      compilerState = null;
    }
    if (compilerState) {
  ...
  // ============================================================================
  const targetOption: string | string[] | undefined = options.target || (config.output as any)?.targets;
  const emissionTargets: string[] | null = targetOption
    ? targetOption === 'all'
      ? ['css', 'tailwind', 'design-tokens', 'figma', 'graph-json']
      : typeof targetOption === 'string'
        ? targetOption.split(',').map((t: string) => t.trim()).filter(Boolean)
  ...
  try {
    ...
    if (emissionTargets && emissionTargets.length > 0) {
      const { emit } = await import('@compiler/pipeline/lowering/emitter-registry.js');
      ...
      if (allIRs.length > 0) {
        const mergedIR = {
          ...
          meta: {
            ...allIRs[0].meta,
            sourceFiles: [...new Set(allIRs.flatMap((ir: any) => ir.meta?.sourceFiles || []))],
          },
        };
        ...
        for (const target of emissionTargets) {
          ...
          try {
            const result = emit(mergedIR, target as any, { minify: options.minify });
            if (result) {
              const targetPath = path.join(outputDir, result.fileName);
              ensureDirectory(path.dirname(targetPath));
              fs.writeFileSync(targetPath, result.output, 'utf8');
              logger.info(`  🎯 ${target}: ${path.relative(process.cwd(), targetPath)} (${(result.bytes / 1024).toFixed(1)}KB)`);
            }
          } catch (err) {
            logger.warn(`  ⚠️ Failed to emit ${target}: ${(err as Error).message}`);
          }
        }
      }
    }
    ...
    if (isPersistent && persistentCache && compilerState) {
      ...
      await saveCompilerStateToDisk(compilerState, persistentCache, projectHash);
  ...
  if (options.watch || config.watch) {
        ...
        if (options.verbose && compilerState) {
          ...
          if (impact) {
            logger.info(
              ...
              (impact.shouldIncremental ? ' — incremental' : ' — full recompile recommended')
            );
          }
        }
          ...
          if (emissionTargets && emissionTargets.length > 0) {
            ...
            if (inspector?.ir) {
              for (const target of emissionTargets) {
                if (target === 'css') continue;
                try {
                  const result = emit(inspector.ir, target as any, { minify: options.minify });
                  if (result) {
                    const targetPath = path.join(outputDir, result.fileName);
                    ensureDirectory(path.dirname(targetPath));
                    fs.writeFileSync(targetPath, result.output, 'utf8');
    ...
    const cleanupAndExit = (): never => {
      ...
      if (compilerState && persistentCache) {
        const projectHash = Buffer.from(process.cwd()).toString('base64').substring(0, 8);
        saveCompilerStateToDisk(compilerState, persistentCache, projectHash).then(() => {
