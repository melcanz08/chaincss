...
import {
  compileRuntime,
  removeRuntimeModule,
  setManifest as setGlobalManifest,
  setTokens as setGlobalTokens,
  ...
  const processStyles = (sourceStyles: Record<string, any>) => {
    ...
    if (Object.keys(injectionBundle).length > 0) {
      ...
      for (const [key, dynamicClass] of Object.entries(dynamicMap)) {
        ...
        finalClassMap[key] = [staticPart, dynamicClass]
          .filter(Boolean)
          .join(" ");
      }
    }
...
export function styled(tag: any, styles: any) {
  return (props: any) => {
    ...
    const combinedClass = () => {
      ...
      return [rootClass, props.class, props.className]
        .filter(Boolean)
        .join(" ");
    };
...
export function useComputedStyles(styleFactory: any, props: Accessor<any>) {
  const computedStyles = createMemo(() => ({
    root: styleFactory(typeof props === "function" ? props() : props),
  }));
...
export function useChainCSSContext() {
  return useContext(ChainCSSContext);
}
export function setManifest(m: any) {
  setGlobalManifest(m);
}
...
export function cx(...classes: any[]) {
  ...
  for (const cls of classes) {
    ...
    if (typeof cls === "string") result.push(cls);
      ...
      for (const [k, v] of Object.entries(cls)) if (v) result.push(k);
    }
  }
  return result.join(" ");
}
...
export function enableSolidDebug() {
  debugEnabled = true;
  if (typeof window !== "undefined")
    (window as any).__CHAINCSS_SOLID_DEBUG__ = true;
}
export function disableSolidDebug() {
  debugEnabled = false;
  if (typeof window !== "undefined")
    (window as any).__CHAINCSS_SOLID_DEBUG__ = false;
}
export function isSolidDebugEnabled() {
  return (
    debugEnabled ||
    (typeof window !== "undefined" &&
      !!(window as any).__CHAINCSS_SOLID_DEBUG__)
  );
}
  ...
  for (const [prop, fn] of Object.entries(styleObj.dynamic)) {
    if (typeof fn === "function") {
      try {
        ...
        console.warn(
          `[ChainCSS Solid] Error evaluating dynamic "${prop}":`,
          err,
        );
      }
    }
