...
async function fetchFigmaVariables(
  ...
  for (const [id, v] of Object.entries(vars)) {
    const name =
      (v as any).name?.replace(/\//g, ".").replace(/\s+/g, "-").toLowerCase() ||
      id;
...
export function figmaSyncPlugin(opts: FigmaSyncOptions = {}): Plugin {
  const mode =
    opts.mode || (opts.url ? "url" : opts.fileId ? "figmaVariables" : "url");
  ...
  async function loadRelationships(): Promise<any[]> {
    try {
      ...
      if (stat.mtimeMs > lastConfigMtime) {
        ...
        cachedRelationships =
          cfg.tokens?.relationships || cfg.entanglement?.relationships || [];
      }
  ...
  async function fetchTokens(): Promise<any> {
    ...
    if (mode === "figmaVariables") {
      if (!opts.fileId || !opts.token)
        throw new Error("figmaVariables mode requires fileId and token");
      return await fetchFigmaVariables(opts.fileId, opts.token);
    }
    if (mode === "figmaApi") {
      throw new Error(
        "figmaApi legacy mode is currently unsupported. Use figmaVariables mode instead.",
      );
    }
  ...
  async function syncOnce() {
    try {
      ...
      let flat: any = raw;
      try {
        const { importFigmaTokens } =
          await import("@compiler/tokens/design-orchestrator.js");
        const isFigmaLike = !!(
          raw.$themes ||
          raw.$metadata ||
      ...
      let outTokens = flat;
      try {
        const { TokenEntanglementEngine } =
          await import("@compiler/tokens/entanglement.js");
      ...
      const absOut = path.isAbsolute(outputFile)
        ? outputFile
        : path.join(root, outputFile);
      fs.mkdirSync(path.dirname(absOut), { recursive: true });
      fs.writeFileSync(absOut, JSON.stringify(outTokens, null, 2), "utf8");
      logOk(
        `Wrote ${path.relative(root, absOut)} (${Object.keys(outTokens).length} tokens)`,
      );

      if (devServer?.ws) {
        devServer.ws.send({
          type: "custom",
          event: "chaincss:tokens-updated",
          data: { file: absOut, hash: h, timestamp: Date.now() },
        });
        devServer.ws.send({
          type: "custom",
        ...
        for (const mod of devServer.moduleGraph.fileToModulesMap.values()) {
          for (const m of mod) {
            if (m.id && m.id.includes(".chain."))
              devServer.moduleGraph.invalidateModule(m);
          }
        }
      }
    } catch (e: any) {
  ...
  return {
    ...
    async buildStart() {
      if (
        ...
        (this as any).meta?.watchMode === false
      ) {
        await syncOnce();
      }
    },
    ...
    configureServer(server: any) {
      ...
      server.middlewares.use("/__figma-sync", async (_req: any, res: any) => {
        ...
        await syncOnce();
        res.end(
          JSON.stringify({ ok: true, hash: lastHash, timestamp: Date.now() }),
        );
      });

      logOk(
        `Watching Figma source (mode=${mode}, poll=${pollMs}ms) -> ${outputFile}`,
      );
    },
