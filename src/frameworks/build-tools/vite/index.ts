...
import { DEFAULT_CONFIG, ENVIRONMENT_PRESETS } from "@shared/constants/index.js";
...
import { saveCompilerStateToDisk, restoreCompilerStateFromDisk } from '@compiler/pipeline/persistent-compiler.js';
...
function writeClassFileContent(classMap: Record<string,string>, dynamicMap: Record<string,any>) {
  ...
  for (const [name, className] of Object.entries(classMap)) {
    ...
    const hasDyn = dyn && Object.keys(dyn).filter((k: string) => !k.startsWith('_')).length > 0;
    if (hasDyn) {
...
export default function chaincssPlugin(options: ChainCSSPluginOptions = {}): Plugin {
  ...
  async function compileFile(chainPath: string, forcedContent?: string) {
    ...
    inspectorStore.clearFileContext(absPath);
    try {
      const results = await compiler.compileVirtualSource(source, absPath) as Record<string, any>;

      if (!results || typeof results !== 'object' || Array.isArray(results)) {
        logError(`compileVirtualSource returned invalid result for ${path.basename(absPath)}`);
      ...
      for (const [name, compileResult] of Object.entries(results) as [string, any][]) {
        ...
        if ((compileResult as any)?.dynamic) dynamicMap[name] = (compileResult as any).dynamic;
  ...
  console.error("[ChainCSS] results type:", typeof results, "value:", results);
  throw e;
}
      if (rulesToRegister.length > 0) { inspectorStore.addAll(rulesToRegister); 
      console.log('[ChainCSS] IR rules collected:', rulesToRegister.length, 'total:', inspectorStore.size) } else { console.log('[ChainCSS] No IR data for:', absPath) };
      ...
      logError(`Compilation failed on ${path.basename(absPath)}: ${(err as Error).message}`);
      throw err;
    }
  }
  async function compileAllStyles() {
    ...
    try { const stat = fs.statSync(cachePath); if (stat.isFile()) fs.unlinkSync(cachePath); } catch {}
    cssFileCache.clear();
    ...
    async function worker() {
      while (index < chainFiles.length) {
        ...
        try {
          ...
          successCount++;
        } catch (err) { logError(`Pre-compile failure on ${path.basename(file)}: ${(err as Error).message}`); }
      }
    }
  ...
  return {
    ...
    async buildStart() {
      ...
      if (compiler && (compiler as any).isPersistentMode?.() && (compiler as any).persistentCache) {
        try {
          const projectHash = crypto.createHash('md5').update(root).digest('hex');
          const savedState = await restoreCompilerStateFromDisk(
            (compiler as any).persistentCache, projectHash
          );
          if (savedState) {
            (compiler as any).setCompilerState?.(savedState);
            if (!silent) console.log('[ChainCSS] Restored compiler state from cache.');
          }
        } catch (e) { /* silent fail — will do full compilation */ }
      }

      if (isProduction) {
        cssFileCache.clear(); _cachedCSS = '';
        try { await compileAllStyles(); } catch (e) { logError(`buildStart production capture failed: ${(e as Error).message}`); }
      }
    },
    configureServer(devServer: ViteDevServer) {
      devServer.httpServer?.once('listening', async () => {
        ...
        if ((compiler as any).isPersistentMode?.() && (compiler as any).persistentCache) {
          const state = (compiler as any).getCompilerState?.();
          if (state) {
            const projectHash = crypto.createHash('md5').update(root).digest('hex');
            saveCompilerStateToDisk(state, (compiler as any).persistentCache, projectHash)
              .catch(() => {});
          }
        }
      });
      ...
      devServer.middlewares.use('/@chaincss/client.js', (_req:any, res:any) => {
      ...
      devServer.middlewares.use('/__chaincss-ir.json', (_req:any, res:any) => {
      ...
      const handleFileChange = async (fp: string) => {
        ...
        try {
          if ((compiler as any)?.invalidateFileCache) (compiler as any).invalidateFileCache(abs);
          const mod = devServer.moduleGraph.getModuleById(abs) 
            || (devServer.moduleGraph as any).getModulesByFile?.(abs)?.values?.().next?.()?.value;
          ...
          logError(`Watcher context update failed for ${path.basename(abs)}: ${(err as Error).message}`);
        } finally {
      ...
      devServer.watcher.on('unlink', (fp: string) => {
        ...
        if (CHAIN_FILE_RE.test(fp) &&!isTmpFile(fp)) {
          ...
          try { if (fs.existsSync(classFile)) fs.unlinkSync(classFile); } catch {}
          try { if (fs.existsSync(cssFile)) fs.unlinkSync(cssFile); } catch {}
          devServer.ws.send({ type: 'custom', event: 'chaincss-update', data: { timestamp: Date.now() } });
        }
      });
    },
    async handleHotUpdate(ctx:any) {
      ...
      if (isGeneratedOutput(filePath) || !CHAIN_FILE_RE.test(filePath) || isTmpFile(filePath)) return;
      ...
      try {
        ...
        if ((compiler as any)?.invalidateFileCache) (compiler as any).invalidateFileCache(filePath);
        ...
        return ctx.modules.filter((m: any) => m.id === filePath || m.file === filePath);
      } catch (err) {
    ...
    async buildEnd() {
      if (compiler && (compiler as any)?.isPersistentMode?.() && (compiler as any)?.persistentCache) {
        ...
        if (state) {
          try {
            const projectHash = crypto.createHash('md5').update(root).digest('hex');
            await saveCompilerStateToDisk(state, (compiler as any).persistentCache, projectHash);
          } catch {}
        }
      }
    },
    resolveId(id: string) {
      ...
      if (id === 'virtual:chaincss-manifest' || id === '\0virtual:chaincss-manifest') {
    ...
    transformIndexHtml() {
      ...
      return [
        { tag: 'style', attrs: { id: 'chaincss-styles', 'data-chaincss': '' }, children: '/* ChainCSS Dev Server Hot Injection Hook */', injectTo: 'head' },
