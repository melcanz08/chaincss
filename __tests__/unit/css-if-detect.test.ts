...
describe('CSS if() Detection', () => {
  ...
  describe('emitCSSIf', () => {
    it('generates native CSS if() for single condition', () => {
      ...
      // Assert updated modern @supports query check
    ...
    it('generates native CSS if() with else-if chain', () => {
      ...
      // Assert flat spec semicolon sequence structure
      expect(css).toContain('font-size: if(style(--size: small): 12px; style(--size: large): 24px; else: 16px);');
