...
import {
  ...
  findRule,
  resetIdCounter,
} from '../../src/style-ir.js';
import type { StyleIR, IRRule } from '../../src/style-ir.js';
...
describe('Style IR System', () => {
  ...
  describe('IR Factory', () => {
    it('creates an empty IR', () => {
      ...
      expect(ir.meta.version).toMatch(/^\d+\.\d+\.\d+/);
  ...
  describe('parseIR — StyleDefinition → IR', () => {
    ...
    it('parses hover pseudo-classes', () => {
      ...
      expect(ir.rules[0].pseudoClasses[0].declarations[0].property).toBe('background-color');
    });
