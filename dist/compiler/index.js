...
var DesignTokens = class _DesignTokens {
  ...
  get(path4, defaultValue = "") {
    if (this.tokenCache.has(path4)) {
      return this.tokenCache.get(path4);
    }
    let value;
    if (path4 in this.customFlattened) {
      value = this.customFlattened[path4];
    }
    if (value === void 0 && path4 in this.defaultFlattened) {
      value = this.defaultFlattened[path4];
    }
    ...
    this.tokenCache.set(path4, result);
    return result;
  }
  // Get token with type safety
  getColor(path4, defaultValue = "#000000") {
    return this.get(`colors.${path4}`, defaultValue);
  }
  getSpacing(path4, defaultValue = "0") {
    return this.get(`spacing.${path4}`, defaultValue);
  }
  getFontSize(path4, defaultValue = "1rem") {
    return this.get(`typography.fontSize.${path4}`, defaultValue);
  }
  getFontWeight(path4, defaultValue = "400") {
    return this.get(`typography.fontWeight.${path4}`, defaultValue);
  }
  getLineHeight(path4, defaultValue = "1.5") {
    return this.get(`typography.lineHeight.${path4}`, defaultValue);
  }
  getBreakpoint(path4, defaultValue = "768px") {
    return this.get(`breakpoints.${path4}`, defaultValue);
  }
  getZIndex(path4, defaultValue = "0") {
    return this.get(`zIndex.${path4}`, defaultValue);
  }
  getShadow(path4, defaultValue = "none") {
    return this.get(`shadows.${path4}`, defaultValue);
  }
  getBorderRadius(path4, defaultValue = "0") {
    return this.get(`borderRadius.${path4}`, defaultValue);
  }
  ...
  has(path4) {
    return path4 in this.customFlattened || path4 in this.defaultFlattened;
  }
...
function getPropertySuggestion(prop, context) {
  ...
  const candidates = context && contextProperties[context] ? contextProperties[context] : COMMON_CSS_PROPERTIES2;
...
function resolveToken(value, useTokens = true, tokenContext) {
  ...
  if (value.includes("$")) {
    return value.replace(/\$([a-zA-Z0-9.-]+)/g, (match, path4) => {
      const resolved = resolveTokenPath(path4, tokenContext);
      if (resolved !== void 0 && resolved !== null) {
        return String(resolved);
      }
      if (process.env.NODE_ENV !== "production") {
        console.warn(`[ChainCSS] Token not found: ${path4}`);
      }
      return match;
    });
  }
...
function resolveTokenPath(path4, tokenContext) {
  let resolved = null;
  if (tokenContext && typeof tokenContext.get === "function") {
    resolved = tokenContext.get(path4);
  }
  if ((resolved === void 0 || resolved === null) && currentTokenContext) {
    resolved = currentTokenContext.get(path4);
  }
  if (resolved === void 0 || resolved === null) {
    if (tokens && typeof tokens.get === "function") {
      resolved = tokens.get(path4);
    }
  }
  return resolved;
}
var TokenResolver = class {
  ...
  context;
  constructor(context) {
    this.context = context || currentTokenContext || tokens;
  }
  resolve(value) {
...
function addBreakpoint(name, query) {
  currentBreakpoints[name] = query;
}
function removeBreakpoint(name) {
...
function setTokenContext2(context) {
...
var AtomicOptimizer = class {
  ...
  generateClassName(prop, value, type) {
    const hash = crypto.createHash("md5").update(`${prop}${value}`).digest("hex").slice(0, 6);
...
`;
  ...
  optimize(styles) {
    ...
    const componentHash = crypto.createHash("md5").update(cleanName + componentId).digest("hex").slice(0, 6);
...
function validateTheme(contract, theme = {}, path4 = "") {
  const errors = [];
...
var CacheManager = class {
  ...
  constructor(cachePath = "./.chaincss-cache", options = {}) {
    ...
    this.cachePath = path2.resolve(process.cwd(), cachePath);
    this.cacheDir = path2.dirname(this.cachePath);
    this.cache = {};
    this.load();
  ...
  load() {
    try {
      if (fs2.existsSync(this.cachePath)) {
        let data = fs2.readFileSync(this.cachePath, "utf8");
  ...
  checkAndPrune() {
    try {
      const stats = fs2.statSync(this.cachePath);
  ...
  save() {
    ...
    try {
      ...
      fs2.writeFileSync(tempPath, data, "utf8");
      fs2.renameSync(tempPath, this.cachePath);
      this.dirty = false;
    } catch (error) {
...
var PersistentCache = class {
  ...
  constructor(options = {}) {
    ...
    this.cacheDir = path3.resolve(process.cwd(), this.options.cacheDir);
    this.metadataPath = path3.join(this.cacheDir, "metadata.json");
  ...
  async getByHash(hash) {
    ...
    const cachePath = path3.join(this.cacheDir, `${hash}.json`);
    if (fs3.existsSync(cachePath)) {
      try {
        const entry = JSON.parse(fs3.readFileSync(cachePath, "utf8"));
        if (!this.isExpired(entry)) {
  ...
  async setByHash(hash, result, dependencies = []) {
    ...
    this.memoryCache.set(hash, entry);
    const cachePath = path3.join(this.cacheDir, `${hash}.json`);
    fs3.writeFileSync(cachePath, JSON.stringify(entry, null, 2));
  ...
  setByHashSync(hash, result, source, dependencies = []) {
    ...
    const depHashes = {};
    for (const dep of dependencies) {
      if (typeof dep === "string" && fs3.existsSync(dep)) {
        const depSource = fs3.readFileSync(dep, "utf8");
  ...
  invalidateByHash(hash) {
    ...
    fs3.writeFileSync(this.metadataPath, JSON.stringify(this.metadata, null, 2));
  }
  // ============================================================================
  ...
  loadMetadata() {
    if (fs3.existsSync(this.metadataPath)) {
      try {
        const data = JSON.parse(fs3.readFileSync(this.metadataPath, "utf8"));
        this.metadata = data;
      } catch (err) {
        this.metadata = { entries: {}, totalSize: 0, lastCleanup: 0 };
      }
  ...
  async updateMetadata(hash, entry) {
    const cachePath = path3.join(this.cacheDir, `${hash}.json`);
    const stats = fs3.statSync(cachePath);
  ...
  async enforceSizeLimit() {
    ...
    for (const entry of entries) {
      ...
      const cachePath = path3.join(this.cacheDir, `${entry.hash}.json`);
      if (fs3.existsSync(cachePath)) {
...
export {
  ...
  setAtomicOptimizer,
  setBreakpoints,
  setBreakpoints as setBreakpointsUtil,
  setSourceComments,
  setTokenContext,
  shorthandMap,
  tokens2 as tokens,
